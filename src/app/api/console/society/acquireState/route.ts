...
import acquireSocietyState
  from "@/app/dependencies/dataBackend/middleware/society/acquireState";

export async function POST(request: Request) {
  try {
    const { society } = await request.json();
    const result = await acquireSocietyState(society);
    if (!result) {
