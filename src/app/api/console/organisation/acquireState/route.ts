...
import acquireOrganisationState
  from "@/app/dependencies/dataBackend/middleware/organisation/aquireState";

export async function POST(request: Request) {
  try {
    const { organisation } = await request.json();
    const result = await acquireOrganisationState(organisation);
    if (!result) {
