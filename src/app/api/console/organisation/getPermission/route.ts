...
import getOrganisationPermission
  from "@/app/dependencies/dataBackend/middleware/organisation/getPermission";

export async function POST(request: Request) {
  try {
    const { uuid } = await request.json();
    const result = await getOrganisationPermission(uuid);
